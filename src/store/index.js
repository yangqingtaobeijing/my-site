...
export const store = reactive({
    articles: loadJSON(KEYS.articles, []),
    bookmarks: loadJSON(KEYS.bookmarks, []),
    projects: loadJSON(KEYS.projects, []),
    config: loadJSON(KEYS.config, { ...DEFAULT_CONFIG }),
});
/** 数据是否正在从远程加载 */
...
export async function loadFromGitHub() {
    ...
    try {
        const [articles, bookmarks, projects, config] = await Promise.all([
            fetchPublicData('articles.json'),
            fetchPublicData('bookmarks.json'),
            fetchPublicData('projects.json'),
            fetchPublicData('config.json'),
        ]);
 ...
 * 返回是否成功
 */
async function syncFileToGitHub(filePath, data, message) {
...
export function getArticles() {
    return [...store.articles].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
}
/** 根据 ID 获取文章 */
...
export async function toggleArticleHidden(id) {
...
export async function deleteArticle(id) {
    ...
    saveJSON(KEYS.articles, store.articles);
    return syncFileToGitHub('data/articles.json', store.articles, `删除文章: ${article?.title || id}`);
}
// ==================== 收藏操作 ====================
/** 获取所有收藏（按创建日期倒序） */
export function getBookmarks() {
    return [...store.bookmarks].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
}
/** 根据 ID 获取收藏 */
...
export async function toggleBookmarkHidden(id) {
...
export async function deleteBookmark(id) {
    ...
    saveJSON(KEYS.bookmarks, store.bookmarks);
    return syncFileToGitHub('data/bookmarks.json', store.bookmarks, `删除收藏: ${bookmark?.title || id}`);
}
// ==================== 项目操作 ====================
/** 获取所有项目（按创建日期倒序） */
...
export async function toggleProjectHidden(id) {
...
export async function deleteProject(id) {
    ...
    saveJSON(KEYS.projects, store.projects);
    return syncFileToGitHub('data/projects.json', store.projects, `删除项目: ${project?.title || id}`);
}
// ==================== 站点配置 ====================
/** 获取站点配置 */
